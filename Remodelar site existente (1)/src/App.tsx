    ...
    <div style={{ minHeight: '100vh', background: '#0D1B2A', fontFamily: 'Inter, sans-serif' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(13,27,42,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 48px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: '#1565D8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, fontWeight: 700, color: '#fff', fontFamily: 'DM Sans, sans-serif' }}>R</div>
          <span style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: 18, color: '#fff', letterSpacing: '-0.3px' }}>RESOLVA</span>
        </div>
      ...
      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', maxWidth: 760, padding: '0 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={onLogin} style={{ padding: '14px 32px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.06)', color: '#fff', cursor: 'pointer', fontSize: 15, fontFamily: 'Inter, sans-serif', fontWeight: 500, backdropFilter: 'blur(8px)' }}>
              Ver Demo
            </button>
          </div>
        </div>
...
function BrandLogo({ size = 38, compact = false }: { size?: number; compact?: boolean }) {
  return (
    <div style={{
      ...
      height: size,
      borderRadius: 14,
      background: 'linear-gradient(135deg, #1e7af6 0%, #6dd5ff 48%, #1b9cff 100%)',
      boxShadow: '0 12px 28px rgba(30, 122, 246, 0.35)',
      display: 'flex',
      alignItems: 'center',
      ...
      <svg width={compact ? size * 0.52 : size * 0.56} height={compact ? size * 0.52 : size * 0.56} viewBox="0 0 64 64" aria-hidden="true" style={{ position: 'relative', zIndex: 1 }}>
        <path d="M32 9L47 14V29C47 40.5 39.8 49.8 32 54C24.2 49.8 17 40.5 17 29V14L32 9Z" fill="rgba(255,255,255,0.2)" stroke="rgba(255,255,255,0.9)" strokeWidth="2.5" />
        <path d="M29 22L38 22L34 31H41L27 48L30 36H23L29 22Z" fill="#fff" />
      </svg>
    </div>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#0D1B2A' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px', position: 'relative', overflow: 'hidden' }}>
        <video src={heroVideo} autoPlay muted loop playsInline style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.25 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(13,27,42,0.7) 0%, rgba(21,58,92,0.8) 100%)' }} />
        <div style={{ position: 'relative', zIndex: 10, maxWidth: 380 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 48 }}>
            <BrandLogo size={42} />
            <div>
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 800, fontSize: 22, color: '#fff', letterSpacing: '0.12em' }}>RESOLVA</div>
              <div style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.6)' }}>Serviços técnicos</div>
            </div>
          </div>
          <h2 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 32, fontWeight: 700, color: '#fff', letterSpacing: '-0.5px', lineHeight: 1.2, marginBottom: 16 }}>
            Gestão técnica<br />profissional.
          </h2>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 40 }}>
            Da solicitação ao pagamento — tudo controlado numa plataforma.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {['📋 Pedidos e orçamentos digitais', '📅 Agendamento sem conflitos', '📸 Registo fotográfico de problemas', '💳 Controlo de pagamentos'].map(item => (
              <div key={item} style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', fontFamily: 'Inter, sans-serif' }}>{item}</div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ width: 480, background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 56px', flexShrink: 0 }}>
        <div style={{ width: '100%', maxWidth: 360 }}>
          <div style={{ marginBottom: 32 }}>
            <h1 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 24, fontWeight: 700, color: '#0D1B2A', margin: '0 0 6px', letterSpacing: '-0.3px' }}>{title}</h1>
...
function LoginPage({ users, onSuccess, onRegister, onForgot, onBack }: { users: UserAccount[]; onSuccess: (u: AuthUser) => void; onRegister: () => void; onForgot: () => void; onBack: () => void }) {
  ...
  return (
    <AuthLayout title="Bem-vindo de volta" subtitle="Inicia sessão na tua conta RESOLVA">
      {error && <div style={{ padding: '10px 14px', borderRadius: 8, background: '#FEF2F2', border: '1px solid #FCA5A5', color: '#DC2626', fontSize: 13, fontFamily: 'Inter, sans-serif', marginBottom: 16 }}>{error}</div>}
      <InputField label="Email" type="email" placeholder="o.teu@email.com" value={email} onChange={setEmail} />
      <div style={{ marginBottom: 18 }}>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#0D1B2A', fontFamily: 'Inter, sans-serif', marginBottom: 6 }}>Palavra-passe</label>
        <input type="password" placeholder="••••••••" value={pass} onChange={e => setPass(e.target.value)} style={{ width: '100%', padding: '11px 14px', borderRadius: 8, border: '1px solid #E4E9F0', fontSize: 14, fontFamily: 'Inter, sans-serif', color: '#0D1B2A', outline: 'none', boxSizing: 'border-box', background: '#FAFBFC' }} onFocus={e => e.target.style.borderColor = '#1565D8'} onBlur={e => e.target.style.borderColor = '#E4E9F0'} />
      </div>
      <button onClick={handleLogin} style={{ width: '100%', padding: '12px', borderRadius: 8, border: 'none', background: '#1565D8', color: '#fff', cursor: 'pointer', fontSize: 15, fontWeight: 600, fontFamily: 'Inter, sans-serif', marginTop: 8, marginBottom: 20 }}>
        Entrar
      </button>
      <div style={{ textAlign: 'center', fontSize: 14, color: '#6B7A90', fontFamily: 'Inter, sans-serif' }}>
        Ainda não tens conta?{' '}
        <button onClick={onRegister} style={{ color: '#1565D8', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, fontFamily: 'Inter, sans-serif' }}>Criar conta</button>
      </div>
...
function RegisterPage({ users, onSuccess, onLogin, onBack }: { users: UserAccount[]; onSuccess: (u: AuthUser) => void; onLogin: () => void; onBack: () => void }) {
...
function PedidosPage({ role, pedidos, onPedidosChange, userName }: { role: Role; pedidos: Pedido[]; onPedidosChange: (next: Pedido[]) => void; userName: string }) {
...
function OrcamentosPage({ role, orcamentos, onOrcamentosChange, pedidos }: { role: Role; orcamentos: Orcamento[]; onOrcamentosChange: (next: Orcamento[]) => void; pedidos: Pedido[] }) {
  ...
  function handleAccept(id: string) {
    ...
    if (target) {
      ...
      if (pedido) {
        const nextPedido = { ...pedido, status: 'aceite' as Status }
        const index = pedidos.findIndex(p => p.id === pedido.id)
        if (index >= 0) {
          const next = [...pedidos]
          next[index] = nextPedido
        }
      }
    }
  }
...
function AgendamentosPage({ role, agendamentos, onAgendamentosChange, disponibilidade, onDisponibilidadeChange, userName }: { role: Role; agendamentos: Agendamento[]; onAgendamentosChange: (next: Agendamento[]) => void; disponibilidade: string[]; onDisponibilidadeChange: (next: string[]) => void; userName: string }) {
  ...
  const visibleAgendamentos = agendamentos.filter(a => {
    if (role === 'cliente') return a.cliente === 'Ana Ferreira'
    if (role === 'tecnico') return a.tecnico === 'Carlos Mendes'
    return true
  })
...
function PagamentosPage() {
  const total = pagamentosData.reduce((s, p) => s + p.valor, 0)
  const pago = pagamentosData.filter(p => p.status === 'pago').reduce((s, p) => s + p.valor, 0)
  const pendente = pagamentosData.filter(p => p.status === 'pendente').reduce((s, p) => s + p.valor, 0)
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
        <StatCard label="Total Faturado" value={`€${total.toLocaleString('pt-PT')}`} icon="💶" color="#EFF6FF" />
        <StatCard label="Recebido" value={`€${pago.toLocaleString('pt-PT')}`} icon="✅" color="#F0FDF4" delta={`${pagamentosData.filter(p => p.status === 'pago').length} pagamentos`} />
        <StatCard label="Pendente" value={`€${pendente.toLocaleString('pt-PT')}`} icon="⏳" color="#FFFBEB" delta={`${pagamentosData.filter(p => p.status === 'pendente').length} por receber`} />
      </div>
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E4E9F0', overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#F8FAFB' }}>
              {['ID', 'Cliente', 'Serviço', 'Valor', 'Data', 'Método', 'Estado'].map(h => (
...
export default function App() {
  ...
  if (screen === 'register') {
    return <RegisterPage users={users} onSuccess={user => { setUsers(prev => [{ nome: user.nome, email: user.email, password: '123456', role: user.role }, ...prev]); handleAuthSuccess(user) }} onLogin={() => setScreen('login')} onBack={() => setScreen('landing')} />
  }
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#F4F6FA' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          ...
          {page === 'pedidos' && <PedidosPage role={user.role} pedidos={pedidos} onPedidosChange={setPedidos} userName={user.nome} />}
          {page === 'orcamentos' && <OrcamentosPage role={user.role} orcamentos={orcamentos} onOrcamentosChange={setOrcamentos} pedidos={pedidos} />}
